...
DEFAULT_COMPOSITE_PROFILE = COMPOSITE_PROFILE_OLD
...
BUILTIN_PALETTES: tuple[PrincePalette, ...] = (
    ...
    PrincePalette(
        "composite-simple",
        "Simple composite — 160×200 DOSBox-X Old CGA artifact colors",
        DOSBOXX_CGA_COMPOSITE_OLD_COLORS,
        note=(
            "DOSBox-X cga_composite (Old CGA), mode 6, foreground F, default hue; "
...
def display_colors(
    ...
    if mode == "composite":
        return DOSBOXX_CGA_COMPOSITE_COLORS
    raise ValueError(f"Unknown display mode: {mode}")
...
def render_display_mode(
    ...
    hardware_palette: PrincePalette | None = None,
    *,
    composite_colors: Sequence[tuple[int, int, int]] = DOSBOXX_CGA_COMPOSITE_COLORS,
    transparent_zero: bool = False,
    checkerboard: bool = True,
    channels: int = 3,
) -> RenderedRaster:
