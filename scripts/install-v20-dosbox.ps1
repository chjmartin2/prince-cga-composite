...
$packageName = "Prince-1.3-New-CGA-V20-Shared-P0-P2-Sword-Dungeon-Version-B-DAT-Set"
$sourcePath = Join-Path $projectRoot "runtime\build\$packageName"
$targetPath = "C:\DOS\POP_CP"
$expectedExeHash = "bce926cc781d2725df917d6bb45bbd67e2e46e912845f32eb7a01887f39fde6c"
$expectedComHash = "f1f013e7841eba67d6d67ee5799d91987ed7c56c09f3701b496f9ebe6fa53ecf"

if (-not (Test-Path -LiteralPath $sourcePath -PathType Container)) {
    throw "V20U package directory not found: $sourcePath"
}
...
if ($manifest.package -ne $packageName) {
    throw "Unexpected V20U package identity: $($manifest.package)"
}
...
$exeHash = (Get-FileHash -LiteralPath (Join-Path $source "P4KX20.EXE") -Algorithm SHA256).Hash.ToLowerInvariant()
$comHash = (Get-FileHash -LiteralPath (Join-Path $source "CGA4K20.COM") -Algorithm SHA256).Hash.ToLowerInvariant()
if ($exeHash -ne $expectedExeHash -or $comHash -ne $expectedComHash) {
    throw "V20U executable or launcher hash mismatch."
}
...
if ($sourceFiles.Count -ne $targetFiles.Count) {
    throw "DOSBox deployment file count differs from V20U package."
}
...
Write-Host "V20U installed exactly to $resolvedTarget" -ForegroundColor Green
Write-Host "$($targetFiles.Count) files verified; run CGA4K20.COM in DOSBox." -ForegroundColor Green
